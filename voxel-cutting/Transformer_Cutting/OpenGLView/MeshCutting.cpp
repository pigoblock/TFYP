...
CString MeshCutting::animateTransformer(CString currBone, bone *rootBone, 
	float tAmt, float rxAmt, float ryAmt, float rzAmt)
{
	...
	for (int i = 1; i < boneArray.size(); i++){
		...
		if (boneArray[i]->m_name == currBone){
			command::print("h\n");
			currBoneFound = true;
			//parent problem
			glPushMatrix();
				glTranslatef(tAmt*boneArray[i]->m_posCoord[0], tAmt*boneArray[i]->m_posCoord[1],
				tAmt*boneArray[i]->m_posCoord[2]);
				drawPolygonFace(boneArray[i]->mesh);

				for (int j = 0; j < boneArray[i]->child.size(); j++){
					drawPolygonFace(boneArray[i]->child[j]->mesh);
				}
			glPopMatrix();
			...
			drawOneTransformerPart(rootBone, boneArray[i]->m_name);
		}
	}

	return rootBone->m_name;
}

void MeshCutting::drawOneTransformerPart(bone *node, CString targetName)
{
	if (node == nullptr){
		return;
	}
