...
IMPLEMENT_DYNCREATE(SuggestionsView2, CView)
BEGIN_MESSAGE_MAP(SuggestionsView2, CView)
	ON_WM_CREATE()
	ON_WM_KEYDOWN()
	ON_WM_SIZE()
	ON_WM_LBUTTONDOWN()
	...
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSEWHEEL()
	ON_COMMAND(ID_COLOR_BACKGROUND, &SuggestionsView2::OnColorBackground)
END_MESSAGE_MAP()

SuggestionsView2::SuggestionsView2(void)
{
	// Set background color 
	bColorIdx = 2;
}

SuggestionsView2::~SuggestionsView2()
{

}

void SuggestionsView2::setID(int id){
	viewId = id;
}

void SuggestionsView2::InitGL()
{
	COpenGL Initgl;
...
void SuggestionsView2::OnDraw(CDC* pDC)
{
	CKEGIESDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc){
...
void SuggestionsView2::DrawView()
{
	CKEGIESDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc){
...
BOOL SuggestionsView2::PreCreateWindow(CREATESTRUCT& cs)
{
	return CView::PreCreateWindow(cs);
}

#ifdef _DEBUG
void SuggestionsView2::AssertValid() const
{
	CView::AssertValid();
}

void SuggestionsView2::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CKEGIESDoc* SuggestionsView2::GetDocument() const // 
{
...
int SuggestionsView2::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void SuggestionsView2::OnInitialUpdate()
{
...
void SuggestionsView2::OnSize(UINT nType, int cx, int cy)
{
...
void SuggestionsView2::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	char lsChar;
	lsChar = char(nChar);
...
void SuggestionsView2::OnTimer(UINT_PTR nIDEvent)
{
...
void SuggestionsView2::OnLButtonDown(UINT nFlags, CPoint point)
{
	LEFT_DOWN = true;
...
void SuggestionsView2::OnLButtonUp(UINT nFlags, CPoint point)
{
	LEFT_DOWN = false;
	CView::OnLButtonUp(nFlags, point);
}

void SuggestionsView2::OnRButtonDown(UINT nFlags, CPoint point)
{
	RIGHT_DOWN = true;
	m_PreMousePos.x = point.x;
	m_PreMousePos.y = -point.y;
...
void SuggestionsView2::OnRButtonUp(UINT nFlags, CPoint point)
{
	RIGHT_DOWN = false;
...
void SuggestionsView2::OnMouseMove(UINT nFlags, CPoint point)
{
...
// Zooming the view
BOOL SuggestionsView2::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	vec3d temp;
...
void SuggestionsView2::drawAxis(bool atOrigin, CCamera* cam)
{
	glPushMatrix();
...
void SuggestionsView2::SetupView()
{
...
void SuggestionsView2::UpdateView()
{
...
void SuggestionsView2::OnColorBackground()
{
	bColorIdx++;
