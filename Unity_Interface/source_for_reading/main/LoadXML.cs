...
public class LoadXML : MonoBehaviour {
	...
	GameObject SomeFunction(string s) {;
		...
		obj_gameobject.renderer.material.color = colors [0];
	...
	void mode(){
		...
		if (outerBox) {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.OuterCube.renderer.enabled = true;
			}
		} else {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.OuterCube.renderer.enabled = false;
			}
		}

		if (meshOpen) {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.mesh.renderer.enabled = true;
			}
		} else {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.mesh.renderer.enabled = false;
			}
		}

		if (meshTransparent) {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.mesh.renderer.material.shader = transparent;
			}
		} else {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.mesh.renderer.material.shader = shader1;
			}
		}

		if (boneOpen) {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.skeletonBone.renderer.enabled = true;
				if (co.Value.getInsideBone() != null)
					co.Value.getInsideBone().renderer.enabled = true;
			}
		} else {
			foreach (KeyValuePair <string, component> co in componentMap){
				co.Value.skeletonBone.renderer.enabled = false;
				if (co.Value.getInsideBone() != null)
					co.Value.getInsideBone().renderer.enabled = false;
			}
		}
		...
		if (sphereOpen) {
			foreach (KeyValuePair <string, component> co in componentMap){
				if (co.Value.skeletonSphere != null){
					co.Value.skeletonSphere.renderer.enabled = true;
				}
				if (co.Value.centerSphere != null){
					co.Value.centerSphere.renderer.enabled = true;
				}
			}
		} else {
			foreach (KeyValuePair <string, component> co in componentMap){
				if (co.Value.skeletonSphere != null)
					co.Value.skeletonSphere.renderer.enabled = false;
				if (co.Value.centerSphere != null){
					co.Value.centerSphere.renderer.enabled = false;
				}
			}
		}
	}
	...
	*/
	...
	// read infor.xml, match mesh with its corresponding template part
	private void uploadMesh(XmlNodeList elemList){
		...
		foreach (XmlNode node in elemList) {
			...
			goodMesh.renderer.material.color = colors[index];
			...
			if (componentMap[boneName].getBoneType() == "side_bone"){
				...
				sysmetricGoodMesh.renderer.material.color = colors[index];
	...
	*/
	...
	/*
		Traverse the skeleton tree and save in correct format.
	*/
