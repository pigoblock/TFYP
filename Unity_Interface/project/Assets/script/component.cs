...
namespace Component
{
	...
	public class component 
	{
		...
		public void cubeUnEnable(){
			...
			OuterCube.GetComponent<Renderer>().enabled = false;	
			OuterCube.GetComponent<Collider>().enabled = false;
			collisionCube.GetComponent<Renderer>().enabled = false;
		...
		public void openCollision(){
			collisionCube.GetComponent<Collider>().enabled = true;
		}
		...
		public void skeletonBoneDraw(Vector3 pos){
			...
			skeletonBone.GetComponent<Collider>().enabled=false;
		}
		...
		public void TorsoInsideDraw(){
			...
			insideBone.GetComponent<Collider>().enabled = false;
		}
			...
			if (skeletonSphere != null){
				...
				if (parent != null) {
					if (parent.getName() != "Torso"){
						...
						//Debug.Log(insideBone.name);
						insideBone.transform.position = (start + end) / 2;
						
						Vector3 direction1 = start - end;
						//Debug.Log(Quaternion.LookRotation(direction1));
